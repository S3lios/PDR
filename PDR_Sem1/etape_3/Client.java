...
////////////////////////////////////////////////////////////
	...
	public static SharedObject getObject(int n) {
		return (objetsPartages != null) ? objetsPartages.get(n) : null;
	}

	public static SharedObject stub(Object o, int id) {
		
		SharedObject so = null;
		
		try {
			// on vérifie que la classe stub existe
			File file = new File("./" + o.getClass().getName() + "_stub.java");
			// sinon on l'écrit et compile
			if (!file.exists()) {
				StubGen.generateur(o.getClass().getName());
				StubGen.compilateur(o.getClass().getName());
			}

			// Récupération du constructeur et création de l'objet stub et on l'ajoute à la liste de SharedObject
			Class<?> c = Class.forName(o.getClass().getName() + "_stub");
			so = (SharedObject) c.getDeclaredConstructor(Object.class, int.class).newInstance(o, id);
			objetsPartages.put(so.id, so);
